...
random intercept models. We motivate the discussion by highlighting some issues
...
with high dimensional categorical variables.  To demonstrate this, consider the
following procedure, which regresses a normally distributed $Y$ against a
totally independent categorical predictor $X$
...
It would make an interesting exercise to work out precise distribution of each
...
$\beta$ and $\sigma$ are estimated using maximum likelihood estimation.  As we
have seen, when $X$ is a high dimensional categorical vector, the linear
model's estimates of $\beta$ are plagued by a high variance even in the most
simple situations.

A random intercept model alleviates this variance by adding an additional
assumption into the data model.  The parameter estimates themselves (referred to
in this context as *random intercepts*) are assumed to come from a normal
distribution, whose parameter estimates the model also estimates. This has a
...
This fixed intercept and slope, random intercept model is also easy to estimate
with `lmer`
...
"Hierarchical Models" is also used, or "Mixed-Effects Models", the naming is
confusing).
...
We can express this idea in the context of a multilevel model by hypothesizing
...
distribution of effective parameters in our model, grouped by their associated
class
...
Breaking this down
  ...
  each group of `subclass` estimates centered around the estimated intercept for the parent `class`.
